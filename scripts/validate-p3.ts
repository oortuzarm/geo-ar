...
function runSection(label: string, questions: Array<{ id: string; q: string; expectedIds: string[] }>) {
  console.log(`\n${'═'.repeat(60)}`)
  console.log(`  ${label}`)
...
// ── VALIDACIONES P3 — 10 CONSULTAS EJECUTIVAS ─────────────────────────────────
const p3 = [
  { id: 'E01', q: '¿Dónde debería invertir primero?',                                   expectedIds: ['goal-decision-making'] },
  { id: 'E02', q: '¿Qué sucursal merece más recursos?',                                  expectedIds: ['goal-decision-making', 'goal-franchises'] },
  { id: 'E03', q: '¿Cómo priorizo mis ubicaciones?',                                     expectedIds: ['goal-decision-making'] },
  { id: 'E04', q: '¿Dónde tengo más oportunidades de crecimiento?',                     expectedIds: ['goal-decision-making'] },
  { id: 'E05', q: '¿Cómo justifico una expansión?',                                      expectedIds: ['goal-decision-making'] },
  { id: 'E06', q: '¿Cómo sé qué ubicación aporta más valor?',                           expectedIds: ['goal-decision-making', 'spatial-concentration'] },
  { id: 'E07', q: '¿Cuál es mi mejor ubicación?',                                        expectedIds: ['goal-decision-making', 'spatial-concentration'] },
  { id: 'E08', q: '¿Cómo decido dónde abrir la siguiente sucursal?',                    expectedIds: ['goal-choose-location'] },
  { id: 'E09', q: '¿Cómo presentar evidencia para una inversión?',                      expectedIds: ['goal-decision-making'] },
  { id: 'E10', q: '¿Cómo defender una decisión frente al directorio?',                  expectedIds: ['goal-decision-making'] },
]

// ── REGRESIÓN 50 originales ────────────────────────────────────────────────────
const orig50: Array<{ id: string; q: string; expectedId?: string; shouldNeverBe?: string }> = [
...
const p2Comercial = [
  ...
  { id: 'C3', q: 'Quiero entender qué sedes reciben más personas', expectedId: 'goal-gyms' },
  ...
  { id: 'C7', q: '¿Cómo medir asistencia de clientes?', expectedId: 'goal-measure-physical' },
]
const p2Educacion = [
...
// Regressions — inline
function runRegression(label: string, items: Array<{ id: string; q: string; expectedId?: string; shouldNeverBe?: string }>) {
  let pass = 0, fail = 0
  const issues: string[] = []
...
totalFail += runRegression('Regresión — 50 preguntas originales', orig50)
totalFail += runRegression('Regresión — P1 (7 preguntas)',
  p1.map(x => ({ ...x, expectedId: x.expectedId })) as any)
totalFail += runRegression('Regresión — P2 comercial (7 preguntas)',
  p2Comercial.map(x => ({ ...x })) as any)
totalFail += runRegression('Regresión — P2 educación (5 preguntas)',
  p2Educacion.map(x => ({ ...x })) as any)
