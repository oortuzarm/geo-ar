...
// ─── POI marker (landmark / point of interest) ────────────────────────────────

function PoiPin({ cx, cy, color }: { cx: number; cy: number; color: string }) {
  return (
    <g>
...
// ─── Visual 1: Content at POI — entry trigger → audio ────────────────────────

export function TourismContentVisual() {
  return (
    <div className="relative rounded-2xl overflow-hidden border border-white/[0.08]" style={{ height: 340, background: '#060910' }}>
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 480 340" preserveAspectRatio="xMidYMid slice">
        ...
        <PoiPin cx={178} cy={175} color={ACCENT} />
        {/* Person approaching */}
      ...
      <div className="absolute" style={{ bottom: 14, right: 14 }}>
        <PhoneWebShell url="exp.ubyca.com/sendero-norte" width={152}>
          <UbycaExperienceCard
            brand="Parque Nacional"
            accent={ACCENT}
            message="Este punto de interés tiene contenido disponible."
            buttons={[{ label: 'Reproducir audio guía', icon: 'audio' }]}
          />
        </PhoneWebShell>
      </div>
    </div>
...
// ─── Visual 2: Sequential route — 3-stop vertical layout ──────────────────────

export function TourismRouteVisual() {
  return (
    <div className="relative rounded-2xl overflow-hidden border border-white/[0.08]" style={{ height: 340, background: '#060910' }}>
      <div className="absolute inset-0 pointer-events-none" style={{
...
// ─── Visual 3: Dwell at museum — minimum time → video ────────────────────────

export function TourismDwellVisual() {
  const pX = 210
  const pY = 160
  const ringR = 24
  const circ = 2 * Math.PI * ringR
  const filled = 0.75 * circ
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden border border-white/[0.08]" style={{ height: 340, background: '#060910' }}>
      ...
      <div className="absolute" style={{ bottom: 14, right: 14 }}>
        <PhoneWebShell url="exp.ubyca.com/museo-arte" width={152}>
          <UbycaExperienceCard
            brand="Museo de Arte"
            accent={ACCENT}
            message="Este espacio tiene contenido disponible ahora."
            buttons={[{ label: 'Ver video', icon: 'video' }]}
          />
        </PhoneWebShell>
      </div>
    </div>
...
// ─── Visual 4: Presence analytics — visitors across POIs ─────────────────────

export function TourismAnalyticsVisual() {
  const dots = [
    { cx: 145, cy: 145 }, { cx: 168, cy: 132 }, { cx: 188, cy: 155 },
    { cx: 155, cy: 172 }, { cx: 178, cy: 162 }, { cx: 198, cy: 148 },
    { cx: 135, cy: 188 }, { cx: 162, cy: 198 }, { cx: 182, cy: 182 },
    { cx: 210, cy: 165 }, { cx: 128, cy: 162 }, { cx: 175, cy: 148 },
  ]
  ...
  return (
    <div className="relative rounded-2xl overflow-hidden border border-white/[0.08]" style={{ height: 340, background: '#060910' }}>
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 480 340" preserveAspectRatio="xMidYMid slice">
        <StreetGrid />
        <circle cx={170} cy={168} r={118} fill="rgba(255,255,255,0.015)" />
        <circle cx={170} cy={168} r={118} fill="none" stroke="rgba(255,255,255,0.09)" strokeWidth={1} strokeDasharray="5 4" />
        <circle cx={170} cy={158} r={62}  fill={`${ACCENT}04`} />
        <circle cx={170} cy={158} r={38}  fill={`${ACCENT}07`} />
        <circle cx={170} cy={158} r={18}  fill={`${ACCENT}12`} />
        {dots.map((d, i) => (
          <g key={i}>
      ...
      <div className="absolute flex flex-col gap-3" style={{ bottom: 16, right: 16 }}>
        <AnalyticsCard label="Visitantes en ruta" value="34" />
        <AnalyticsCard label="Promedio por parada" value="14" unit="min" accent={ACCENT} />
      </div>
    </div>
  )
}
