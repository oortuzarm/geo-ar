...
function normalizeProject(raw: Record<string, unknown>): GeoProject {
  return {
    ...
    projectLogoPositionX:     raw.projectLogoPositionX     ?? raw.project_logo_position_x,
    projectLogoPositionY:     raw.projectLogoPositionY     ?? raw.project_logo_position_y,
...
export class RemoteGeoRepository implements IGeoRepository {
  ...
  async syncProject(id: string, project: Partial<GeoProject>, points: GeoPoint[]): Promise<GeoProject> {
    ...
    const serializedPoints = points.map((p) => ({
      ...
      point_logo_url:        p.pointLogoUrl        ?? null,
    ...
    const raw = await apiFetch<Record<string, unknown>>(this.url(`/api/geo_projects/${id}/sync`), {
      ...
      body: JSON.stringify({
        ...project,
        project_logo_url:        project.projectLogoUrl        ?? null,
        project_logo_zoom:       project.projectLogoZoom        ?? null,
        project_logo_position_x: project.projectLogoPositionX  ?? null,
        project_logo_position_y: project.projectLogoPositionY  ?? null,
