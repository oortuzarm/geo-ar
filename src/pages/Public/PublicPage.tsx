...
/** Gentler alpha when deriving heading from route bearing (less noisy source) */
const NAV_BEARING_ALPHA = 0.1
/** How many route waypoints ahead to look when computing fallback bearing */
const NAV_BEARING_LOOKAHEAD = 4

/** Returns the compass bearing (0–360 °) from one point to another. */
...
export default function PublicPage() {
  ...
  // ── Navigation mode ────────────────────────────────────────────────────────
  // isNavigating: true while the turn-by-turn nav HUD is active
  // navLockMode:  'follow' = auto-pan + heading rotation; 'free' = user panned manually
  // navHeading:   smoothed compass bearing used for CSS map rotation (0–360°)
  // hasCompass:   true once the device delivers at least one orientation reading
  const [isNavigating,  setIsNavigating]  = useState(false)
  const [navLockMode,   setNavLockMode]   = useState<'follow' | 'free'>('follow')
  const [navHeading,    setNavHeading]    = useState(0)
  const [hasCompass,    setHasCompass]    = useState(false)
  const navHeadingRef = useRef<number>(0)   // mutable, never stale in callbacks
  ...
  useEffect(() => {
    ...
    function onOrientation(e: DeviceOrientationEvent) {
      // iOS provides webkitCompassHeading (0–360, CW from north, always absolute).
      // Android / generic: use alpha. alpha is CCW from north on many devices,
      // so heading = (360 - alpha) % 360 gives a CW-from-north value.
      const raw =
  ...
  // ── Bearing fallback — drives heading when no compass is available ─────────
  // Uses the vector from user → next route waypoint. Updates on every GPS fix
  // so the "camera" slowly rotates to match actual movement direction.
  useEffect(() => {
    if (!isNavigating || hasCompass || !routeResult || !userLocation) return

    const trimmed = trimRouteFromUser(
      routeResult.latLngs,
    ...
    const idx = Math.min(NAV_BEARING_LOOKAHEAD, trimmed.length - 1)
    const [toLat, toLng] = trimmed[idx]
    const bearing = calculateBearing(userLocation.latitude, userLocation.longitude, toLat, toLng)

    const smoothed = smoothHeading(navHeadingRef.current, bearing, NAV_BEARING_ALPHA, NAV_HEADING_THRESHOLD)
    if (smoothed === null) return

    navHeadingRef.current = smoothed
    setNavHeading(smoothed)
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [userLocation?.latitude, userLocation?.longitude, isNavigating, hasCompass, routeResult])
  ...
  async function handleStartNavigation() {
    ...
    // requestPermission returns 'granted' | 'denied'. On error or denied we
    // still start navigation — just without compass rotation (bearing fallback).
  ...
  function handleNavUserDrag() {
    // User manually panned the map — switch to free mode.
    // Zero out heading immediately so the map is axis-aligned
    // and touch coordinates are correct while exploring.
    setNavLockMode('free')
    setNavHeading(0)
    navHeadingRef.current = 0
  }
  ...
  function handleNavRecenter() {
    ...
    if (userLocation) {
      flyToCounterRef.current += 1
      setFlyToKey(`nav-recenter-${flyToCounterRef.current}`)
      setFlyToTarget({ lat: userLocation.latitude, lng: userLocation.longitude, zoom: 18 })
    }
  }
  ...
  return (
    <div className="bg-gray-950 overflow-hidden relative md:flex md:flex-col" style={{ height: '100dvh' }}>
      ...
      <div className="absolute inset-0 md:relative md:flex-1 overflow-hidden">
        ...
        <div
          ...
          style={
            isNavigating
              ? navLockMode === 'follow'
                ? {
                    transform: `perspective(900px) rotateX(20deg) rotate(${-navHeading}deg)`,
                ...
                : {
                    // Free mode: instant snap to 0° so touch coords stay correct
                    transform: 'none',
                    transition: 'none',
                  }
              : {}
        ...
        >
          <MapContainer center={mapCenter} zoom={15} className="w-full h-full">
            ...
            <NavigationMapController
              ...
              isLocked={navLockMode === 'follow'}
              userLocation={userLocation}
              onUserDrag={handleNavUserDrag}
            />
            <TileLayer
        ...
        {/* Share button — hidden during navigation */}
        <button
        ...
        {/* My location button — hidden during navigation (auto-follow handles centering) */}
        <button
      ...
      {/* ── NAVIGATION BOTTOM CARD ──────────────────────────────────────────
          Persistent HUD while navigating. Hides all other mobile bottom UI. */}
      {isNavigating && selectedPoint && (
