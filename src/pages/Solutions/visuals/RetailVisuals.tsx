// ─── Retail use-case visual panels — v2 ──────────────────────────────────────
// Distinct visual identity per case — no two look alike.
// Case 1: map + phone (entry trigger)
// Case 2: phone dominant + map ultra-faint (loyalty reward)
// Case 3: two territories + phone (competitor conquesting)
// Case 4: analytics dashboard — no phone, data is the hero
    ...
    <div style={{ background: '#07090f' }}>
      ...
      <div style={{ padding: '10px 12px 8px' }}>
        ...
        <div style={{ display: 'flex', gap: 2.5 }}>
          {[...Array(8)].map((_, i) => (
            <div key={i} style={{
              flex: 1, height: 5.5, borderRadius: 2.5,
              background: i < 7 ? ACCENT : `${ACCENT}18`,
              opacity: i < 7 ? 0.55 + i * 0.07 : 1,
            }} />
          ))}
        </div>
      </div>
...
// Composition: map + geofence + phone
// Story: person crosses boundary → welcome notification fires
...
export function RetailEntryVisual() {
  return (
    ...
    >
      ...
      <div className="absolute flex items-center gap-2" style={{ left: 12, bottom: 12 }}>
        <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border" style={{ background: `${ACCENT}0c`, borderColor: `${ACCENT}28` }}>
          <div className="w-1.5 h-1.5 rounded-full" style={{ background: ACCENT }} />
          <span style={{ fontSize: 9, fontFamily: 'monospace', fontWeight: 600, color: `${ACCENT}cc`, letterSpacing: '0.04em' }}>
...
// Composition: large phone (hero) + ultra-faint map behind
// Story: presence detected → points credited instantly in the app
...
// Composition: two territories + phone
// Story: person inside competitor zone → YOUR brand fires the intercept
...
// Composition: full-bleed map + heat zones + metric cards
// No phone — the store manager is the audience, not the customer
// Story: real-time operational intelligence → data-driven decisions

export function RetailAnalyticsVisual() {
  // People per heat zone — color-matched to their zone
  ...
  return (
    <div
      ...
      style={{ height: 340, background: '#060910' }}
    >
      {/* Map — more visible here; it IS the interface */}
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 480 340" preserveAspectRatio="xMidYMid slice">
        <StreetGrid />

        {/* Store perimeter — subtle outer boundary */}
        <circle cx={215} cy={178} r={135} fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth={1} strokeDasharray="4 4" />

        {/* ── Heat zone 1: Entrada (orange / hot) ── */}
        ...
        {/* ── Heat zone 2: Temporada (blue / medium) ── */}
      ...
      >
        <div className="flex items-center gap-2">
          <div className="relative" style={{ width: 8, height: 8 }}>
            <div className="absolute inset-0 rounded-full" style={{ background: '#4ade80', opacity: 0.35 }} />
        ...
        <span style={{ fontSize: 8, color: 'rgba(255,255,255,0.18)', fontFamily: 'system-ui' }}>
          Nova Moda · Av. Corrientes
        </span>
      </div>
      ...
      {[
        ...
        >
          <p style={{ fontSize: 7.5, color: 'rgba(255,255,255,0.30)', marginBottom: 3, fontFamily: 'system-ui' }}>
            {card.label}
          </p>
          <p style={{ fontSize: 20, fontWeight: 900, color: card.valueColor, lineHeight: 1, fontFamily: 'system-ui', letterSpacing: '-0.01em' }}>
            {card.value}
          </p>
          <p style={{ fontSize: 7, color: card.deltaColor, marginTop: 3, fontFamily: 'system-ui' }}>
            {card.delta}
          </p>
        </div>
      ))}
      ...
      {/* ── Zone labels on map ── */}
      <div
        className="absolute flex items-center gap-1.5 rounded-md"
        style={{ top: 94, left: 210, padding: '3px 7px', background: 'rgba(251,146,60,0.10)', border: '1px solid rgba(251,146,60,0.22)' }}
      >
        <div style={{ width: 5, height: 5, borderRadius: '50%', background: 'rgb(251,146,60)', flexShrink: 0 }} />
        <span style={{ fontSize: 7.5, fontFamily: 'monospace', fontWeight: 700, color: 'rgba(251,146,60,0.85)', whiteSpace: 'nowrap' }}>
          ENTRADA · 8.2 min
        </span>
      </div>
      ...
      <div
        className="absolute flex items-center gap-1.5 rounded-md"
        style={{ top: 252, left: 96, padding: '3px 7px', background: `${ACCENT}0f`, border: `1px solid ${ACCENT}25` }}
      >
        <div style={{ width: 5, height: 5, borderRadius: '50%', background: ACCENT, flexShrink: 0 }} />
        <span style={{ fontSize: 7.5, fontFamily: 'monospace', fontWeight: 700, color: `${ACCENT}cc`, whiteSpace: 'nowrap' }}>
          TEMPORADA · 14.1 min
        </span>
      </div>
      ...
      >
        {[
          { label: 'Retorno 30d', value: '64%' },
          { label: 'Activos ahora', value: '18' },
          { label: 'Pico del día', value: '13:00h' },
        ].map((s, i) => (
          <div key={i} style={{ flex: 1, textAlign: i === 0 ? 'left' : i === 2 ? 'right' : 'center' }}>
            <p style={{ fontSize: 7, color: 'rgba(255,255,255,0.25)', fontFamily: 'system-ui', marginBottom: 2 }}>{s.label}</p>
            <p style={{ fontSize: 12, fontWeight: 800, color: 'rgba(255,255,255,0.80)', fontFamily: 'system-ui', lineHeight: 1 }}>{s.value}</p>
          </div>
        ))}
      </div>
    </div>
