...
function runSection(label, questions) {
  console.log(`
${"\u2550".repeat(60)}`);
  console.log(`  ${label}`);
  console.log("\u2550".repeat(60) + "\n");
  let pass = 0, fail = 0;
  for (const { id, q, expectedIds } of questions) {
    const audit = auditMatch(q);
    const actualId = audit.winner?.id ?? "FALLBACK";
    const ok = expectedIds.includes(actualId);
    const cand = allCandidates.find((c) => c.id === actualId);
    const activated = cand ? activatedKeywords(cand.keywords, q) : [];
    if (ok) pass++;
    else fail++;
    const icon = ok ? "\u2705" : "\u274C";
    const suffix = ok ? "" : ` \u2192 esperado: [${expectedIds.join(" | ")}]`;
    console.log(`${icon} ${id} | match=${actualId} score=${audit.winner?.score ?? 0}${suffix}`);
    console.log(`   Q: "${q}"`);
  ...
  console.log(`Resultado: ${pass}/${pass + fail}${fail > 0 ? " \u274C" : " \u2705"}`);
  return fail;
}
var p3 = [
  { id: "E01", q: "\xBFD\xF3nde deber\xEDa invertir primero?", expectedIds: ["goal-decision-making"] },
  { id: "E02", q: "\xBFQu\xE9 sucursal merece m\xE1s recursos?", expectedIds: ["goal-decision-making", "goal-franchises"] },
  { id: "E03", q: "\xBFC\xF3mo priorizo mis ubicaciones?", expectedIds: ["goal-decision-making"] },
  { id: "E04", q: "\xBFD\xF3nde tengo m\xE1s oportunidades de crecimiento?", expectedIds: ["goal-decision-making"] },
  { id: "E05", q: "\xBFC\xF3mo justifico una expansi\xF3n?", expectedIds: ["goal-decision-making"] },
  { id: "E06", q: "\xBFC\xF3mo s\xE9 qu\xE9 ubicaci\xF3n aporta m\xE1s valor?", expectedIds: ["goal-decision-making", "spatial-concentration"] },
  { id: "E07", q: "\xBFCu\xE1l es mi mejor ubicaci\xF3n?", expectedIds: ["goal-decision-making", "spatial-concentration"] },
  { id: "E08", q: "\xBFC\xF3mo decido d\xF3nde abrir la siguiente sucursal?", expectedIds: ["goal-choose-location"] },
  { id: "E09", q: "\xBFC\xF3mo presentar evidencia para una inversi\xF3n?", expectedIds: ["goal-decision-making"] },
  { id: "E10", q: "\xBFC\xF3mo defender una decisi\xF3n frente al directorio?", expectedIds: ["goal-decision-making"] }
];
var orig50 = [
...
var p2Comercial = [
  ...
  { id: "C3", q: "Quiero entender qu\xE9 sedes reciben m\xE1s personas", expectedId: "goal-gyms" },
  ...
  { id: "C7", q: "\xBFC\xF3mo medir asistencia de clientes?", expectedId: "goal-measure-physical" }
];
var p2Educacion = [
...
var totalFail = 0;
totalFail += runSection("VALIDACIONES P3 \u2014 VOCABULARIO EJECUTIVO (10 preguntas)", p3);
function runRegression(label, items) {
  let pass = 0, fail = 0;
  const issues = [];
  for (const { id, q, expectedId, shouldNeverBe } of items) {
    const audit = auditMatch(q);
    const actualId = audit.winner?.id ?? "FALLBACK";
    if (expectedId && actualId !== expectedId) {
      fail++;
      issues.push(`  \u274C ${id}: esperado="${expectedId}" got="${actualId}"`);
    } else if (shouldNeverBe && actualId === shouldNeverBe) {
      fail++;
      issues.push(`  \u274C ${id}: no debe ser "${shouldNeverBe}" \u2014 got="${actualId}"`);
    } else {
      pass++;
    }
  }
...
${"\u2500".repeat(60)}`);
  ...
  console.log(`Resultado: ${pass}/${pass + fail}${fail > 0 ? " \u274C REGRESIONES" : " \u2705"}`);
  return fail;
}
totalFail += runRegression("Regresi\xF3n \u2014 50 preguntas originales", orig50);
totalFail += runRegression(
  "Regresi\xF3n \u2014 P1 (7 preguntas)",
  p1.map((x) => ({ ...x, expectedId: x.expectedId }))
);
totalFail += runRegression(
  "Regresi\xF3n \u2014 P2 comercial (7 preguntas)",
  p2Comercial.map((x) => ({ ...x }))
);
totalFail += runRegression(
  "Regresi\xF3n \u2014 P2 educaci\xF3n (5 preguntas)",
  p2Educacion.map((x) => ({ ...x }))
);
console.log(`
${"\u2550".repeat(60)}`);
