...
// scripts/audit-vocab.ts
var queries = [
  // 1. Performance de sucursales
  "Tengo 20 sucursales. \xBFC\xF3mo s\xE9 cu\xE1les realmente est\xE1n aportando valor al negocio?",
  "Quiero saber cu\xE1les de mis tiendas generan m\xE1s inter\xE9s entre los clientes.",
  "\xBFQu\xE9 sucursal funciona mejor?",
  "\xBFQu\xE9 local genera mejores resultados?",
  "\xBFCu\xE1l aporta m\xE1s valor?",
  // 2. Campañas
  "Mi jefe me pidi\xF3 demostrar si una campa\xF1a presencial tuvo impacto.",
  "\xBFC\xF3mo demuestro que una activaci\xF3n funcion\xF3?",
  "\xBFC\xF3mo justifico una campa\xF1a en terreno?",
  "\xBFC\xF3mo s\xE9 si tuvo resultados?",
  // 3. Operaciones
  "\xBFC\xF3mo puedo verificar que mis contratistas visitaron los puntos asignados?",
  "\xBFC\xF3mo controlo a proveedores en terreno?",
  "\xBFC\xF3mo verifico cuadrillas?",
  "\xBFC\xF3mo s\xE9 si personal externo fue al lugar?",
  // 4. Turismo
  ...
  "Quiero crear una experiencia de exploraci\xF3n urbana.",
  "Quiero un recorrido guiado.",
  // 5. Apps existentes
  "Si yo ya tengo una app y una base de clientes, \xBFpara qu\xE9 necesitar\xEDa Ubyca?",
  "Ya tengo usuarios y una aplicaci\xF3n m\xF3vil.",
  "\xBFQu\xE9 agrega Ubyca a mi app?",
  "\xBFPor qu\xE9 integrarlo?",
  // 6. Comparación con QR
  "\xBFPor qu\xE9 usar Ubyca en lugar de una landing con QR?",
  "\xBFQu\xE9 diferencia hay con un QR?",
  "\xBFPor qu\xE9 no usar simplemente QR?",
  "\xBFPor qu\xE9 no una p\xE1gina con QR?"
];
for (let i = 0; i < queries.length; i++) {
  const a = auditMatch(queries[i]);
  const winner = a.winner ? `[${a.winner.type}] ${a.winner.id}  score=${a.winner.score}  keys=[${a.winner.matchedKeywords.join(" | ")}]` : "\u26A0 FALLBACK";
  console.log(`
